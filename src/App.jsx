...
const COMPANIES = [
  ...
  { name: "GDP", color: "#1A1A40" },
  { name: "Yard", color: "#0EA5E9" },
...
const SEED = [
  ...
  { id: "e2", day: "mon", sort: 721, time: "12:00–12:30 PM", companies: ["C2", "Kepler", "kyu"], host: "Brands & Culture Villa", title: "", who: "Tina Blakeney (C2), Toby Hack (Kepler) · moderated by Michael Kania (kyu)", venue: "13 Avenue de Benefiat", notes: "", type: "panel", link: "https://www.brandsnculture.com/bc-cannes/partner-event-mon-22/" },
  ...
  { id: "e15", day: "wed", sort: 780, time: "1:00–1:45 PM", company: "GhostNote", host: "Inkwell Beach", title: "License to Lead: The Human Side of AI", who: "Reggie Snowden", venue: "HuraCannes — 2 Rue Félix Faure, 06400 Cannes", notes: "", type: "panel", link: "" },
...
const ATTENDEES = [
  { name: "Tina Blakeney", company: "C2" },
  { name: "Thomas Ellingson", company: "GDP" },
  { name: "Steven Jumper", company: "GhostNote" },
  { name: "Reggie Snowden", company: "GhostNote" },
  { name: "Mariko Fujimoto", company: "HDY" },
  { name: "Mariko Watannabe", company: "kyu" },
  { name: "Kentaro Kimura", company: "kyu" },
  { name: "Michael Kania", company: "kyu" },
  { name: "Perry Tell", company: "Sid Lee" },
  { name: "Marie-Lise Campeau", company: "Sid Lee" },
  { name: "Johan Vakidis", company: "Sid Lee" },
  { name: "Marianne Pizzi", company: "Sid Lee" },
  { name: "JF Lavigne", company: "Sid Lee" },
  { name: "David Allard", company: "Sid Lee" },
  { name: "Aurélia Gimenez", company: "Sid Lee" },
  { name: "Tom Brunet", company: "Yard" },
  { name: "Ambroise Soulé", company: "Yard" },
  { name: "Güllit Baku", company: "Yard" },
  { name: "Camm Rowland", company: "Kepler" },
  { name: "Remy Stiles", company: "Kepler" },
  { name: "Garrett Dale", company: "Kepler" },
  { name: "Isabella Benayoun", company: "Kepler" },
  { name: "Jess Haley", company: "Kepler" },
  { name: "Toby Hack", company: "Kepler" },
];
...
export default function App() {
  ...
  const attendeeGroups = useMemo(() => {
    const m = {};
    ATTENDEES.forEach(({ name, company }) => { (m[company] = m[company] || new Set()).add(name); });
    return Object.entries(m).map(([c, s]) => [c, Array.from(s)]).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
  }, []);
  const attendeeCount = useMemo(() => new Set(ATTENDEES.map((a) => a.name)).size, []);
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: "radial-gradient(1100px 460px at 78% -120px, rgba(244,183,64,0.22), transparent), linear-gradient(180deg,#FBF6EC, #F7EFE0)", fontFamily: "ui-sans-serif, system-ui, -apple-system, sans-serif", color: "#2A2118" }}>
      <style>{`
        ...
        .hero-anim{background-size:200% 200%;animation:heroShift 22s ease-in-out infinite}
        .grain{position:absolute;inset:0;background-image:${GRAIN};background-size:160px 160px;opacity:.08;mix-blend-mode:overlay;pointer-events:none}
        .evt-card{transition:transform .18s ease, box-shadow .18s ease}
        .evt-card:hover{transform:translateY(-2px);box-shadow:0 12px 26px -14px rgba(20,40,60,.4)}
      ...
      <header className="relative overflow-hidden text-white hero-anim" style={{ background: "linear-gradient(155deg,#08415C 0%,#0E7C9B 24%,#1CA9C9 40%,#E8825A 72%,#F4B740 100%)" }}>
        <div className="grain" />
        <div className="absolute right-2 -top-6 pointer-events-none" style={{ opacity: 0.22 }}><Constellation size={250} nodes={12} color="#FFFFFF" /></div>
        <Palmtree className="absolute left-1 -bottom-3 opacity-15" size={94} strokeWidth={1} style={{ transform: "rotate(-8deg)" }} />
        <Palmtree className="absolute right-6 -bottom-4 opacity-10" size={78} strokeWidth={1} style={{ transform: "scaleX(-1) rotate(-6deg)" }} />
        <div className="relative max-w-6xl mx-auto px-5 pt-5 pb-5">
          <div className="flex items-center gap-2.5">
            <KyuMark size={32} />
            <span className="text-xs font-semibold tracking-[0.22em] uppercase" style={{ opacity: 0.95 }}>collective <span style={{ opacity: 0.5 }}>×</span> Cannes Lions</span>
          </div>
          <h1 className="mt-2 flex flex-wrap items-baseline gap-x-3" style={{ fontFamily: SERIF }}>
      ...
      <section className="max-w-6xl mx-auto px-5 mt-8">
        <div className="relative rounded-2xl border overflow-hidden p-5 sm:p-7" style={{ borderColor: "#E5DFD0", background: "linear-gradient(180deg,#FBFAF6,#F5F1E7)" }}>
          <div className="absolute -right-10 -top-10 pointer-events-none hidden sm:block" style={{ opacity: 0.16 }}><Constellation size={230} nodes={11} color={KYU_BLUE} /></div>
          <div className="relative">
            <div className="text-xs font-bold uppercase tracking-[0.22em]" style={{ color: KYU_BLUE }}>The collective, on the ground</div>
            <h2 className="mt-1 text-2xl sm:text-3xl" style={{ fontFamily: SERIF, color: "#231C12" }}>Who's in Cannes</h2>
            <p className="text-sm mt-1" style={{ color: "#8A7C66" }}>{attendeeCount} people from across the kyu collective at the festival.</p>
            <div className="mt-5 grid sm:grid-cols-2 gap-x-10 gap-y-3.5">
              {attendeeGroups.map(([company, names]) => (
                <div key={company} className="flex items-baseline gap-2">
                  <span className="flex items-center gap-1.5 shrink-0 font-semibold" style={{ color: colorFor(company), fontSize: "13px" }}>
                    <span className="w-2 h-2 rounded-full" style={{ background: colorFor(company) }} /> {company}
                  </span>
                  <span className="flex-1 self-center border-b border-dotted mx-1" style={{ borderColor: "#D8CEB8" }} />
                  <span className="text-right text-sm" style={{ fontFamily: SERIF, fontStyle: "italic", color: "#5C513F" }}>{names.join(", ")}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* CONTACT FLAG */}
      <footer className="max-w-6xl mx-auto px-5 mt-6 pb-16">
        ...
        <div className="flex items-center justify-center gap-2 mt-6">
          <KyuMark size={20} />
          <span className="text-xs" style={{ color: "#BCAE91" }}>collective · Cannes Lions 2026</span>
        </div>
      </footer>
    </div>
  );
...
function KyuMark({ size = 32 }) {
  return (
    <span className="inline-flex items-center justify-center rounded-full shrink-0" style={{ width: size, height: size, background: "radial-gradient(circle at 35% 30%, #AEB9EE, #586AC4)", fontFamily: SERIF, fontSize: size * 0.42, color: "#fff", boxShadow: "0 2px 12px rgba(70,90,190,.45)", lineHeight: 1 }}>kyu</span>
  );
}

function Constellation({ size = 220, nodes = 12, color = "#6E7FD4" }) {
  ...
  const lines = [];
  for (let i = 0; i < nodes; i++) for (let j = i + 1; j < nodes; j++) lines.push([pts[i], pts[j]]);
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none">
      {lines.map(([a, b], k) => <line key={k} x1={a[0]} y1={a[1]} x2={b[0]} y2={b[1]} stroke={color} strokeWidth="0.4" />)}
      {pts.map(([x, y], k) => <circle key={k} cx={x} cy={y} r="1.6" fill={color} />)}
    </svg>
  );
}
