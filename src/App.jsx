...
import { MapPin, Clock, Users, PartyPopper, Utensils, Sparkles, Coffee, Award, Sun, ExternalLink, Palmtree, Star, Presentation, Mail } from "lucide-react";
...
const SEED = [
  { id: "e1", day: "mon", sort: 660, time: "11:00–11:30 AM", company: "Kepler", host: "Equativ Yacht", title: "Creative at the Speed of Performance", who: "Camm Rowland", venue: "Equativ Yacht", notes: "", type: "panel", link: "https://equativatcannes.com/" },
  ...
  { id: "e4", day: "mon", sort: 810, time: "1:30–2:00 PM", company: "Kepler", host: "Brands & Culture Villa", title: "Marketing at Cruising Altitude", who: "Jess Haley, Gabriella Neudecker & Michael Kania", venue: "13 Avenue de Benefiat", notes: "kyu moderator", type: "panel", link: "https://www.brandsnculture.com/bc-cannes/partner-event-mon-22/" },
  ...
  { id: "e6", day: "tue", sort: 1290, time: "9:30 PM", company: "HDY", host: "HDY Reception", title: "", who: "", venue: "TBD", notes: "", type: "dinner", link: "" },
  ...
  return (
    <div className="min-h-screen w-full" style={{ background: "radial-gradient(1100px 460px at 78% -120px, rgba(244,183,64,0.22), transparent), linear-gradient(180deg,#FBF6EC, #F7EFE0)", fontFamily: "ui-sans-serif, system-ui, -apple-system, sans-serif", color: "#2A2118" }}>
      ...
      <header className="relative overflow-hidden text-white hero-anim" style={{ background: "linear-gradient(155deg,#08415C 0%,#0E7C9B 24%,#1CA9C9 40%,#E8825A 72%,#F4B740 100%)" }}>
        <div className="grain" />
        <div className="absolute -right-12 -top-14 opacity-30"><Sun size={230} strokeWidth={0.7} /></div>
        <Palmtree className="absolute left-1 -bottom-2 opacity-20" size={130} strokeWidth={1} style={{ transform: "rotate(-8deg)" }} />
        <Palmtree className="absolute right-8 -bottom-3 opacity-15" size={104} strokeWidth={1} style={{ transform: "scaleX(-1) rotate(-6deg)" }} />
        <div className="relative max-w-6xl mx-auto px-5 pt-10 pb-9">
          <div className="flex items-center gap-2 text-xs font-semibold tracking-[0.25em] uppercase" style={{ opacity: 0.95 }}>
            <Star size={12} fill="#FCE9B8" stroke="#FCE9B8" />
          ...
          <h1 className="mt-3 leading-[0.95]" style={{ fontFamily: SERIF }}>
            <span className="block text-5xl sm:text-7xl font-medium tracking-tight" style={{ textShadow: "0 2px 18px rgba(0,0,0,.18)" }}>Cannes Lions</span>
            <span className="block text-3xl sm:text-5xl font-light italic mt-1" style={{ color: "#FCE9B8" }}>2026</span>
          </h1>
          <p className="mt-3 text-sm sm:text-base max-w-xl" style={{ opacity: 0.95 }}>The full week of kyu collective activity on the Croisette — speaker moments and the marquee gatherings, all in one place.</p>
          <div className="mt-5 flex flex-wrap gap-2.5">
            <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-medium"><Sun size={13} /> 22–26 June</span>
            <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-medium"><MapPin size={13} /> {FESTIVAL.location}</span>
            <span className="glass inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-medium"><Sparkles size={13} /> {allEvents.length} moments · {companiesPresent.length} companies</span>
          </div>
        </div>
        <div className="h-1.5 w-full" style={{ background: "linear-gradient(90deg,#F4B740,#E8825A 45%,#1CA9C9)" }} />
      </header>

      <section className="px-5 mt-5 max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-2 gap-3">
          <span className="text-xs font-bold uppercase tracking-[0.18em]" style={{ color: "#A9874A" }}>Filter by company</span>
          <div className="flex rounded-full border overflow-hidden text-xs font-semibold shrink-0" style={{ borderColor: "#E7DCC6" }}>
            <button onClick={() => setCompact(true)} className="px-3 py-1 transition" style={{ background: compact ? "#08415C" : "transparent", color: compact ? "#fff" : "#7A6A4D" }}>Compact</button>
            <button onClick={() => setCompact(false)} className="px-3 py-1 transition" style={{ background: !compact ? "#08415C" : "transparent", color: !compact ? "#fff" : "#7A6A4D" }}>Detailed</button>
          </div>
        </div>
      ...
      <main className="mt-5">
        <div className="max-w-6xl mx-auto px-5">
          <div className="flex gap-3 overflow-x-auto pb-3 snap-x">
            {DAYS.map((d, di) => (
              <div key={d.key} className="snap-start shrink-0 sm:flex-1 fade-up" style={{ width: compact ? 172 : 240, minWidth: 150, animationDelay: `${di * 60}ms` }}>
                ...
                <div className="rounded-b-2xl border border-t-0 p-1.5 min-h-[150px]" style={{ borderColor: "#EEE2CC", background: "rgba(255,255,255,.65)" }}>
                  <div className={compact ? "space-y-1.5" : "space-y-2.5"}>
                    {byDay[d.key].length === 0 ? <div className="text-center text-xs py-8" style={{ color: "#CDBFA6" }}>— no moments —</div> : byDay[d.key].map((e) => <MiniCard key={e.id} e={e} compact={compact} />)}
                  </div>
                </div>
              </div>
            ))}
...
function MiniCard({ e, compact }) {
  ...
  const hasMore = e.title || e.who || e.venue || e.notes || e.link;
  return (
    <div className="evt-card rounded-xl border bg-white" style={{ borderColor: "#F0E6D2", borderLeft: `3px solid ${c}`, boxShadow: "0 2px 8px -6px rgba(20,40,60,.25)", cursor: compact && hasMore ? "pointer" : "default", padding: compact ? "8px 9px" : "12px" }}
      onClick={() => compact && hasMore && setOpen((o) => !o)}>
      <div className="flex items-center justify-between gap-1">
        <span className="font-bold flex items-center gap-1" style={{ color: "#5A4A33", fontSize: compact ? "11px" : "12px" }}><Clock size={11} /> {e.time}</span>
        <span className="inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 font-semibold" style={{ fontSize: "10px", background: "#F6EDDA", color: "#9A7330" }}>
          <Icon size={10} />{!compact && <span>{T.label}</span>}
        </span>
      </div>
      ...
      {e.host && <div className="mt-0.5 font-semibold leading-snug" style={{ fontFamily: SERIF, color: "#231C12", fontSize: compact ? "13px" : "15px" }}>{e.host}</div>}
      {showDetail && (
        <>
          {e.title && <div className="mt-0.5 text-xs font-medium" style={{ color: "#5C513F" }}>{e.title}</div>}
          {e.who && <div className="mt-1 text-xs" style={{ color: "#8A7C66" }}>{e.who}</div>}
      ...
      {compact && !open && hasMore && <div className="mt-0.5" style={{ fontSize: "9px", color: "#C9BB9D", letterSpacing: "0.04em" }}>TAP FOR DETAILS</div>}
    </div>
  );
}
